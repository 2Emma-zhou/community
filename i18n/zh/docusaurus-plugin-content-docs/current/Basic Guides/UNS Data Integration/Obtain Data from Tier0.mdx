...
3. 单击刚创建的事件流程，拖拽一个 `mqtt in` 节点并配置参数：
    ...
    - 主题：需获取数据的主题（例如 `factory/workshop/order`）。
...
4. 拖拽一个 `函数` 节点至画布，编写脚本将获取到的数据按第三方接收API的要求格式化：
```js
// 若负载为字符串类型，则解析为JSON
...
6. 添加 `HTTP请求` 或 `mqtt out` 节点，将查询到的数据传输至第三方。